...
@onready var icon_mouse = %TowerPreview
...
func setup(tower: Node, upgrade: TowerUpgradeData) -> void :
    ...
    #If the image exists updates the Display Texture
    print(target_tower.display_image_scene)
    if target_tower and "display_image_scene" in target_tower:
        icon_mouse.texture = target_tower.display_image_scene
        icon_mouse.visible = true
        icon_mouse.modulate = Color.WHITE
