...
func _load_tower_database() -> void :
    CollectionData.tower_database = CollectionData.towers.duplicate(true)
    for tid in CollectionData.tower_database:
        CollectionData.tower_database[tid]["unlocked"] = tid in ProgressManager.unlocked_towers


func _load_enemy_database() -> void :
    CollectionData.enemy_database = CollectionData.enemies.duplicate(true)
    for eid in CollectionData.enemy_database:
        var data = CollectionData.enemy_database[eid]
...
func _populate_towers() -> void :
    var sorted_ids = CollectionData.tower_database.keys()
    sorted_ids.sort()
    for tid in sorted_ids:
        _create_tower_button(CollectionData.tower_database[tid])


func _populate_enemies() -> void :
    var sorted_ids = CollectionData.enemy_database.keys()
    sorted_ids.sort()
    for eid in sorted_ids:
        _create_enemy_button(CollectionData.enemy_database[eid])
