...
func restart_game() -> void :
    ...
    for tower in get_tree().get_nodes_in_group("sentries"):
        tower.queue_free()
...
func _count_amount_of_towers(DesiredTower: String) -> float:
    var counter = 0.0
    var all_towers = get_tree().get_nodes_in_group("towers")

    for tow in all_towers:
        if tow.get("tower_name") == DesiredTower:
            counter += 1

    print("Znaleziono wież: ", counter)
    return counter
