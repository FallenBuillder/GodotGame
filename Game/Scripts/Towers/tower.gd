...
@export var display_image_scene: Texture2D = null
...
func apply_upgrade(upgrade: TowerUpgradeData) -> void :
    ...
    print(upgrade.new_tower_sprite_idle)
    print(upgrade.new_tower_sprite_shoot)

    if upgrade.new_tower_sprite_idle and upgrade.new_tower_sprite_shoot:
        var node = get_node_or_null("Sprite2D")
        if node:
            if upgrade.new_tower_sprite_idle:
                node.texture = upgrade.new_tower_sprite_idle
...
func _tick_frost(_delta: float) -> void :
    ...
    for enemy in enemies_in_range:
        ...
        for other in get_tree().get_nodes_in_group("enemies"):
            if not is_instance_valid(other) or other == enemy or other.is_in_group("towers"):
                continue
